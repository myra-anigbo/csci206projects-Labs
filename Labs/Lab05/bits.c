...
{
    /* TODO: IMPLEMENT THIS */
    int shift = n << 3; //32 bits in total (shift in according to n
    int extracted_byte = ((x >> (shift)) & 0x000000FF); 
    return extracted_byte;
    /*the amount of x shifts to right depends on the amount
     * of n(n=0, shift none, n=1, shift 2 and so on) then
     * using that x will move to the left that amount and your byte would be at the end */ 
}
...
{
    /* TODO: IMPLEMENT THIS */
    int shift_to_end = (x >> 31); // shifts leading bit to end
    int check_zero = shift_to_end & 1; // checks to see if bit is 0 or 1 see if non zero,  
    return !check_zero && x;/*checks to see if the integer in the parameter is 1) negative and 2) not zero (conditions to make an inetger positive)
                              * if 1 !check_zero --> False, automatically makes expression False
                              *if int x =0 !check_zero --> True and !x --> Tr
                              * if positive !check_zero --> True and !x --> True
                              */
}
